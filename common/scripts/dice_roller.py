...
#!nix-shell -i python3 -p "python312.withPackages (ps: with ps; [ numpy tkinter ])"
...
def die_roller(d_type: int, amount: int, modifier: int) -> Tuple[int, int]:
    ...
    max_damage: int = d_type * amount + modifier
    return result, max_damage


def domi_normal(amount:int, to_hit_modifier:int, fire:int, additional:int):
    global LAST_ZERO, LAST_CRIT, TOTAL_CRIT, TOTAL_ZERO, TOTAL_ROLLS
    to_hit, _ = die_roller(20, 1, to_hit_modifier)
    damage, max_damage = die_roller(10, amount, additional)
...
def option_select(option_index):
    match option_index:
        case 0:
            domi_normal(2, 22, 6, 7)
        case 1:
            pass  # Logic for option_index == 1
        case 2:
            pass  # Logic for option_index == 2
        case 3:
            pass  # Logic for option_index == 3
        case _:
            pass  # Default case
