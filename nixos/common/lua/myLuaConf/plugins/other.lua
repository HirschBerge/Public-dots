...
local file_path = vim.fn.expand("$HOME") .. "/.dotfiles/" .. get_hostname() ..
    "/gpt.txt.gpg"

require("chatgpt").setup({ api_key_cmd = "gpg --decrypt " .. file_path })
...
vim.api.nvim_create_autocmd({ 'DirChanged', 'WinEnter', 'BufEnter' }, {
    pattern = '*',
    callback = function()
        vim.fn.system('zellij action rename-pane "' ..
...
vim.api.nvim_set_keymap('n', '<leader>tw', ':lua ToggleTwilight()<CR>',
